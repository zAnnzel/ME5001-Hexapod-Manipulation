...
class Body:
    ...
    #
    def __init__(self, f=0.1115, s=0.13097, m=0.1115) -> None:
...
class Leg:
    ...
    #  |------- y axis
    # origin
    def __init__(self, lengths=[0.05, 0.246, 0.2305],
...
class Hexapod:
    ...
    def solve_ik(self, rot, trans):
        ...
        for k, leg in self.legs.items():
            alpha, beta, gamma = leg.solve_ik(np.array(self.body.vertices[k].get_coordinates()),
                                              np.array(self.ground_contact_points[k].get_coordinates()))
            alpha = format((alpha * math.pi / 180), '.6f')
            beta = format((beta * math.pi / 180), '.6f')
            gamma = format((gamma * math.pi / 180), '.6f')
            array = [alpha, beta, gamma]
            traj.append(array)
        traj = np.array(traj).reshape(1, 18)
        return traj
...
if __name__ == "__main__":
    ...
    # robot.update_dimensions([1, 1, 1, 3, 3, 1])
    # robot.solve_ik([0.1, 0, 0], [0, 0, 0])

    def inverse_kinematics(rx, ry, rz, tx, ty, tz):
        tx *= robot.body.f
        ty *= robot.body.s
        tz *= robot.legs[0].lengths[-1]
        return robot.solve_ik([rx, ry, rz], [tx, ty, tz])

    points = inverse_kinematics(0.26, 0, 0, 0, 0.1, 0)
    print(points)